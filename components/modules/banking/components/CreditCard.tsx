...
import { BgGradient } from '@/components/landing/bg/bg-gradient'
...
const SIZE_MAP = {
  xs: { w: "w-44", h: "h-24", px: "px-2", py: "py-1", text: "text-[9px]" },
  ...
  return (
    <Card
      ...
      className={clsx(
        ...
        s.px,
        s.py,
        "rounded-2xl text-white shadow-lg flex flex-col justify-between transition-transform transform-gpu    hover:shadow-3xl",
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          {logo || institutionLogo ? (
            <Avatar className={`shadow-lg ${s.w.replace("w-", "w-[")}`} style={{ width: 28, height: 28 }}>
              <AvatarImage
       ...
       {/* 3D Chip */}
      <div className=" relative  w-6 h-4 bg-gradient-to-tr from-yellow-400 to-yellow-200 rounded-md shadow-inner grid grid-cols-2 gap-1">
        <div className="absolute border border-gray-900 rounded w-3 h-3 left-1.5 top-0.5" />
     ...
     <div className={clsx("flex flex-col leading-tight text-gray-300", s.text)}>
              {name} 
     ...
     <div className="leading-tight">
        <div className={clsx("uppercase text-gray-300 text-[8px] tracking-tight" )}>Balance</div>
        <CurrencyDisplay amountUSD={balance} variant="small" />
      </div>
