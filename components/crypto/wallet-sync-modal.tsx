...
export function WalletSyncModal({
  ...
  const { realtimeSyncConnected, realtimeSyncError } = useCryptoStore();
  ...
  const isActive = ['queued', 'syncing', 'syncing_assets', 'syncing_transactions', 'syncing_nfts', 'syncing_defi'].includes(status);
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Syncing {walletName || 'Wallet'}
          </DialogTitle>
          <DialogDescription>
            {isCompleted ? 'Sync completed successfully!' :
             isFailed ? 'Sync failed. Please try again.' :
             'Please wait while we sync your wallet data...'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {isCompleted && isAutoClosing && (
            <div className="text-center text-xs text-muted-foreground bg-green-50 p-2 rounded border border-green-200">
              ✅ Sync completed! This dialog will close automatically in 3 seconds...
            </div>
          )}
